  ...
  methods: {
    ...
    async createOrderAndProduct() {
      ...
      try {
        ...
        const userResponse = await axios.get(`http://localhost:8080/users/mail/${this.$store.state.user}/`, { headers });

        console.log("userResponseData",userResponse.data.id)
        console.log("price",this.total)
        console.log("fecha",this.fechaTexto)
        const orderResponse = await axios.post('http://localhost:8080/orders/create', {
        ...
        for (let i = 0; i < this.cart.length; i++) {
          ...
          const productOrderResponse = await axios.post('http://localhost:8080/productsOrder/create', {
